...
namespace Rendering
{
	...
	void ObjectDiffuseLight::UpdateDirectionalLight(const GameTime& gameTime)
	{
		...
		if (mKeyboard->IsKeyDown(DIK_I) && directionalIntensity < UCHAR_MAX)
		{
		...
		if (mKeyboard->IsKeyDown(DIK_O) && directionalIntensity > 0)
		{
			directionalIntensity -= LightModulationRate * elapsedTime;
	...
	void ObjectDiffuseLight::UpdateAmbientLight(const GameTime& gameTime)
	{
		...
		{
			if (mKeyboard->IsKeyDown(DIK_U) && ambientIntensity < UCHAR_MAX)
			{
			...
			if (mKeyboard->IsKeyDown(DIK_P) && ambientIntensity > 0)
			{
				ambientIntensity -= LightModulationRate * (float)gameTime.ElapsedGameTime();
